...
def load_data(data_name, n, batch_size=32):
    if data_name == 'CIFAR10':
        ...
        testset = torchvision.datasets.MNIST(root='./data', train=False,
                                               download=True, transform=data_tf)
        trainset, validateset, _ = torch.utils.data.random_split(trainset, [n, 6000, 54000-n])
        ...
        trainset, testset, _ = torch.utils.data.random_split(trainset, [n, int(len(trainset)*0.2), len(trainset) - int(len(trainset)*0.2) - n])
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True)
