...
def main():
    torch.manual_seed(1)
    np.random.seed(1)
    k_par = 0.08 #param['kernel_par'] # 0.1 for kernel
    ...
    m_arr = (np.logspace(-0.31, 0, base=4, num=30) * n).astype(int)
    ...
    for r in range(repeat):
        ...
        for (i, lambda_reg) in enumerate(lambda_arr):
            for (j, m) in enumerate(m_arr):
                ...
                X_train_phi, y_pred = rff.rf_regression(X_train, y_train.float(), X_test, m, lambda_reg, 'ROS', device, torch.float)
    ...
    result_path = '{}/exp2_ros_{}.pkl'.format(result_dir, 'mnist')
